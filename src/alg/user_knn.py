"""
This file contain the UserKNN recommender which performs
an user-based collaborative filtering algorithm to determine the ranking
...
class UserKNN(RecSys):
    """User k-nearest-neighbours recommender.

    Recommends items to users exploiting users' similarities.
    ...
    def run(self, targets=None, k=10):
        ...
        s = cosine_similarity(dataset.T, alpha=self.alpha, asym=self.asym, h=self.h, knn=self.knn, qfunc=self.qfunc,
                              dtype=np.float32)
        print("elapsed: {:.3f}s\n".format(timer() - start))
    ...
    def evaluate(self, train_set="train_set", test_set="test_set", k=10):
        ...
        start = timer()
        # Compute similarity matrix
        s = cosine_similarity(train_set.T, alpha=self.alpha, asym=self.asym, h=self.h, knn=self.knn, qfunc=self.qfunc,
                              dtype=np.float32)
